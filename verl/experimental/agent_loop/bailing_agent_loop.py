...
class AsyncLLMServerManager:
    ...
    - Load balance: least requests load balancing (default)
    - Sticky session: send multi-turn chat completions to same server for automatic prefix caching
    - Query-count-based load balancing: dispatch to the server with the fewest currently running queries
    - Concurrency control: limit maximum concurrent requests per server
    """

    def __init__(self, config: DictConfig, server_handles: List[ray.actor.ActorHandle], max_cache_size: int = 10000, max_concurrent_per_server: Optional[int] = None):
        ...
        Args:
            ...
            max_concurrent_per_server (int, optional): maximum concurrent requests per server. 
                If None, will try to get from config or use a default value.
        """
        self.config = config
        ...
        # 记录每个server当前正在运行的查询数量
        self.server_running_queries = {server: 0 for server in server_handles}
        
        # 设置每个server的最大并发数量
        if max_concurrent_per_server is None:
            # 尝试从配置中获取，如果没有则使用默认值
            self.max_concurrent_per_server = getattr(config.actor_rollout_ref.rollout, 'max_concurrent_per_server', 128)
        else:
            self.max_concurrent_per_server = max_concurrent_per_server
            
        self._lock = asyncio.Lock()  # 用于并发保护

    async def _choose_server_by_running_queries(self) -> ray.actor.ActorHandle:
        """
        选择当前正在运行查询数量最少的server，同时确保不超过最大并发限制。
        如果所有server都达到最大并发，则等待直到有server可用。
        """
        while True:
            async with self._lock:
                # 找到未达到最大并发的server中，运行查询数最少的
                available_servers = [(server, count) for server, count in self.server_running_queries.items() if count < self.max_concurrent_per_server]
                
                if available_servers:
                    # 在可用的server中选择运行查询数最少的
                    min_count = min(count for _, count in available_servers)
                    candidates = [server for server, count in available_servers if count == min_count]
                    server = random.choice(candidates)
                    self.server_running_queries[server] += 1  # 预分配
                    return server
                else:
                    # 所有server都达到最大并发，需要等待
                    pass
            
            # 释放锁，等待一段时间后重试
            await asyncio.sleep(0.1)  # 等待100ms后重试

    async def _release_server_query(self, server):
        async with self._lock:
            self.server_running_queries[server] -= 1
            if self.server_running_queries[server] < 0:
                self.server_running_queries[server] = 0  # 防御性
    ...
    async def generate(
        ...
        *,
        ...
        Args:
            ...
            use_query_count_balancing (bool): 是否使用基于查询数的负载均衡
        Returns:
            List[int]: List of generated token ids.
        """
        if use_query_count_balancing:
            server = await self._choose_server_by_running_queries()
            try:
                ...
                await self._release_server_query(server)
        else:
    ...
    def get_server_status(self) -> Dict[str, Any]:
        """获取所有server的状态信息，用于监控和调试"""
        return {
            'server_running_queries': {str(server): count for server, count in self.server_running_queries.items()},
            'max_concurrent_per_server': self.max_concurrent_per_server,
            'total_servers': len(self.server_handles),
            'total_running_queries': sum(self.server_running_queries.values()),
        }
...
class AgentLoopMetrics(BaseModel):
    ...
    generative_reward: float = 0.0
    math_verify_reward: float = 0.0
    code_sandbox_reward: float = 0.0
...
class AgentLoopWorker:
    ...
    def get_agent_loop_class(self, agent_name: str) -> Type[AgentLoopBase]:
        ...
        from verl.experimental.agent_loop.bailing_grm_agent_loop import BailingGRMAgentLoop
        if agent_name == "single_turn_agent":
            return BailingGRMAgentLoop
        raise ValueError(f"Only support single turn agent now, Unknown agent_name: {agent_name}")
